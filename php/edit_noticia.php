...
<?php
	...
	if($_FILES['ds_img']['size'] == 0 && $_FILES['ds_img_2']['size'] == 0){
		// Upadate
			try {
			  $att_noticia = $conn->prepare("UPDATE tb_noticia SET nm_noticia = '$nm_noticia', ds_noticia = '$ds_noticia' WHERE id = '$id'");
			  $att_noticia->execute();
			  echo "<script type='text/javascript'>window.location.reload(true);</script>";
			} catch(PDOException $e) {
			    echo $e;
			...
			if($ext == '.jpg' || $ext == 'jpeg' || $ext == '.png'){	
				try {
					if (move_uploaded_file($ds_img['tmp_name'], '../img/'.$nomeFinal)) {
						// Upadate
						$att_noticia = $conn->prepare("UPDATE tb_noticia SET nm_noticia = '$nm_noticia', ds_noticia = '$ds_noticia', id_categoria = '$categoria', img_1 = '$nomeFinal' WHERE id = '$id'");
						$att_noticia->execute();
						echo "<script type='text/javascript'>window.location.reload(true);</script>";
					}else{
						echo 'erro';
			...
			if($ext_2 == '.jpg' || $ext_2 == 'jpeg' || $ext_2 == '.png'){	
				try {
					if (move_uploaded_file($ds_img_2['tmp_name'], '../img/'.$nomeFinal_2)) {
						// Upadate
						$att_noticia = $conn->prepare("UPDATE tb_noticia SET nm_noticia = '$nm_noticia', ds_noticia = '$ds_noticia', id_categoria = '$categoria', img_2 = '$nomeFinal_2' WHERE id = '$id'");
						$att_noticia->execute();
						echo "<script type='text/javascript'>window.location.reload(true);</script>";
					}else{
						echo 'erro';
			...
			if($ext == '.jpg' || $ext == 'jpeg' || $ext == '.png' || $ext_2 == '.jpg' || $ext_2 == 'jpeg' || $ext_2 == '.png'){	
				try {
					if (move_uploaded_file($ds_img['tmp_name'], '../img/'.$nomeFinal) && move_uploaded_file($ds_img_2['tmp_name'], '../img/'.$nomeFinal_2)) {
						// Upadate
						$att_noticia = $conn->prepare("UPDATE tb_noticia SET nm_noticia = '$nm_noticia', ds_noticia = '$ds_noticia', id_categoria = '$categoria', img_1 = '$nomeFinal', img_2 = '$nomeFinal_2' WHERE id = '$id'");
						$att_noticia->execute();
						echo "<script type='text/javascript'>window.location.reload(true);</script>";
					}else{
						echo 'erro';
