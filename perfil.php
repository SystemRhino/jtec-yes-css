	...
	<div>
		...
		<div>
			<div class="mt-2">
				...
				<button data-bs-toggle="collapse" data-bs-target="#config<?php echo $user['id']; ?>">config</button>
			</div>
...
<div class="collapse mt-3" data-bs-parent="#accordion" id="config<?php echo $user['id']; ?>">
	<div class="container col-8">
 		<span id="edit"></span>
		<h5>Editar User #<b id="id_<?php echo $user['id']; ?>"><?php echo $user['id']; ?></b></h5>
		<input class="form-control" type="text" id="nm_user_<?php echo $user['id']; ?>" placeholder="Nome" value="<?php echo $user['nm_user']; ?>">
		<input class="form-control" type="text" id="ds_login_<?php echo $user['id']; ?>" placeholder="E-mail" value="<?php echo $user['ds_login']; ?>">
		<div id="edit-user">
		<div class="col-6">
			<label>Senha Antiga:</label>
			<input class="form-control" type="password" id="<?php echo $user['id'];?>" placeholder="Senha" value="<?php echo $user['ds_senha']; ?>" disabled>
		</div>
		<div class="col-6">
			<label>Nova Senha:</label>
			<input class="form-control" type="password" id="ds_senha_<?php echo $user['id']; ?>" placeholder="Senha" value="<?php echo $user['ds_senha']; ?>">
		</div>
		</div>
		<select class="form-control" id="id_nivel_<?php echo $user['id']; ?>">
			<option value="<?php echo $user['id_nivel']; ?>">
			<?php if($user['id_nivel'] == 1){ 
					echo "Admin";
				}else{ 
					echo "User";
		...
		data-bs-toggle="collapse" data-bs-target="#config<?php echo $user['id']; ?>" aria-controls="collapse">Cancelar</button>
		<button type="button" class="btn btn-primary" id="edit_user_<?php echo $user['id']; ?>">Salvar Alterações</button>
	</div>
	</div>
</div>
		...
		$(document).ready(function(){
			$("#edit_user_<?php echo $user['id']; ?>").click(function(){
  			$.ajax({
  				...
  				type: "POST",
  				data: "nm_user="+$("#nm_user_<?php echo $user['id']; ?>").val()+"&ds_login="+<?php echo $user['id'];?>+"&ds_login="+$("#ds_login_<?php echo $user['id']; ?>").val()+"&ds_senha="+$("#ds_senha_<?php echo $user['id']; ?>").val()+"&id_nivel="+$("#id_nivel_<?php echo $user['id']; ?>").val()+"&id="+<?php echo $user['id'];?>,
  				dataType: "html"
